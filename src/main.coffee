...
register_buttons = ()->
  $.get('src/sign_in_out.php', (data)->
    ...
    $('#login').click ->
      console.log "login"
      user = encrypt($('#user').val())
      pass = encrypt($('#pass').val())
      $.ajax
        url: "src/crypt.php?"
        dataType: "json"
        type: "POST"
    ...
    $('#register').click ->
      ...
      $.ajax
        ...
        success: (response) ->
          user = decrypt(response.user)
          pass = decrypt(response.pass)
          # Logging
    ...
    $('#logout').click ->
      ...
      $.ajax
        ...
        type: "POST"
        data:
          logout: encrypt('logout')

        success: (response) ->
          res = decrypt(response.logout)
